...
const Expiries = () => {
  ...
  return (
    <Box sx={{ height: "auto", borderRadius: "12px", backgroundColor: "background.paper", border: 1, borderColor: "divider" }}>
      ...
      <Box sx={{ display: "flex", height: "fit-content", px: 1.5, pb: 1.5 }}>
        <FormControl sx={{ m: 0, width: "100%" }} size="small">
          ...
          <Select
            ...
            input={<OutlinedInput id="selected-expiry-chip" sx={{ borderRadius: '12px' }} />}
            renderValue={(chosen) => (
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                {chosen.map((expiry) => (
                  <Chip key={expiry} label={expiry} sx={{ fontSize: "11px", borderRadius: '8px' }} />
                ))}
              </Box>
            )}
            MenuProps={MenuProps}
